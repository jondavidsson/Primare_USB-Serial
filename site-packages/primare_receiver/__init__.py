"""
Primare has an RS232 interface to control the receiver.
...
from primare_receiver.primare_commands import CMDS
...
_LOGGER.debug("Loading Primare python package")

class PrimareReceiver(object):
    """Primare receiver."""
    ...
    def exec_command(self, domain, function, operator, value=None):
        ...
        with self.lock:
            ...
            final_command = ''.join(['\x02', cmd, '\x10\x03']).encode('utf-8')
            _LOGGER.debug ('Send Command %s',final_command)
    
            self.ser.write(final_command)
    
            msg = self.ser.read_until(bytes('\x03'.encode()))

        _LOGGER.debug ('Response msg %s', msg.decode())

        split_string = msg.decode().strip().split('W')
